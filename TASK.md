...
Login returns both an access and refresh token rather than a single JWT,
along with a safe user object to avoid an additional round trip.

## JWT Token Generation

- **AccessToken**: includes the subject (id) and useful claims such as email
- **RefreshToken**: includes only the subject (id) — only used to issue a
  new AccessToken once it expires

Two separate secrets are used so access and refresh tokens cannot be swapped
for each other. There is no golden rule on what claims to include — tokens can
be customized to carry more user information if needed.
...
Integration tests cover the API endpoints using Supertest with a real
Inversify DI container and mocked repository/service dependencies:

- `POST /register` — successful registration, password hashing,
  duplicate email, all validation errors
- `POST /login` — successful login, JWT token validation,
  password not exposed in response, generic error message
  for both invalid email and password to prevent email enumeration

### PasswordManagerService tests

- Password hashing and comparison
- Salt randomness — same password produces different hashes
- Timing attack resistance via consistent results
- Edge cases: unicode, whitespace, empty password,
  invalid hash format, very long passwords
